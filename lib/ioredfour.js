...
class Lock {
    constructor(options) {
        ...
        // to issue commands.
        assert(
            options.redis || options.cluster,
            'Must provide Redis configuration or cluster configuration (connection string, options object, or client instance.).'
        );
        ...
        if (options.redis && typeof options.redis.duplicate === 'function') {
            ...
            assert(this._redisSubscriber, 'Provided Redis client must support duplicate() or provide redisSubscriber.');
    ...
    _resolveClusterConfig(options) {
        let clusterOptions =
            options.cluster && typeof options.cluster === 'object'
                ? this._withShardedSubscribers(options.cluster.options)
                : this._withShardedSubscribers(options.clusterOptions);
        ...
        if (options.cluster === true) {
            assert(Array.isArray(options.redis), 'When cluster=true, provide startup nodes array as `redis`.');
            return {
                nodes: options.redis,
                options: clusterOptions
            };
    ...
    acquireLock(id, ttl) {
        ...
        return pipeline.exec().then(([r1, r2]) => {
            ...
            if (useReplication && replications < this._minReplications) {
                ...
                this._redisConnection[this.fn('releaseLock')](this._lockKey(id), lock.index);
            }
